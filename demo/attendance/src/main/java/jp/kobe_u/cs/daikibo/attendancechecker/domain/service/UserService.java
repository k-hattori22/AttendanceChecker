...
public class UserService implements UserDetailsService {
     ...
     * @param userId
     */
    public void changeToPresent(String userId) {
        User user = getUser(userId);
        AttendanceStatus status = user.getStatus();
        //ステータスが出席の場合はエラー
        if (status == AttendanceStatus.PRESENT) {
            throw new AttendanceCheckerException(AttendanceCheckerException.INVALID_USER_STATUS, userId + ": User is already present");
        }
     ...
     * @param userId
     */
    public void changeToAbsent(String userId) {
        User user = getUser(userId);
        AttendanceStatus status = user.getStatus();
        //ステータスが欠席の場合はエラー
        if (status == AttendanceStatus.ABSENT) {
            throw new AttendanceCheckerException(AttendanceCheckerException.INVALID_USER_STATUS, userId + ": User is still absent");
        }
    ...
    public void registerAdmin(String userId, String password) {
        User user = new User(userId, passwordEncoder.encode(password), "システム管理者", null, null,
                null, AttendanceStatus.ABSENT, User.Role.ADMIN, new Date(), new Date());
        uRepo.save(user);
    }
    ...
    public void registerCardReader(String userId, String password) {
        User user = new User(userId, passwordEncoder.encode(password), "カードリーダー", null, null,
                null, AttendanceStatus.ABSENT, User.Role.CARDREADER, new Date(), new Date());
        uRepo.save(user);
    }
}
